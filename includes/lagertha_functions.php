...
function hostPull($terms) {
	
   if ($terms == NULL){
   	$query = "SELECT * FROM hosts ORDER BY hostname"; 
   	}
	else {
		$query = "SELECT * FROM hosts WHERE hostname LIKE '%" . $terms . "%'"; 	
		}	
	...
	if(!$rows) {
		echo "<h2>No Registered Hosts</h2><br />";
	} else {
   ...
   <table class='table table-striped table-hover'>	
	...
	while ($row = mysql_fetch_array($data_p)) {
		...
		<td>"; 		
		if(($curtime-$tasktime) > 300) {     // Check and display if task is active
		...
		<input type='hidden' name='host_id' value='" . $row['hostid'] . "'/>
		<input type='hidden' name='host_name' value='" . $row['hostname'] . "'/>
		<input type='hidden' name='mac_addr' value='" . $row['mac'] . "'/>
      		<button type='submit' class='btn btn-xs btn-info'>Create Task</button>
		</form>
		</td>    
		</tr>
		";
...
// Pulls Active Tasks from DB


function taskPull($host_id) {
	
   if ($host_id == NULL){
   	$query = "SELECT * FROM tasks WHERE pending = 1 ORDER BY taskid"; 
   	}
	else {
   ...
   <table class='table table-striped'>	
	...
	while ($row = mysql_fetch_array($data_p)) {
		...
		<button type='submit' class='btn btn-xs btn-danger' name='cancel'>Cancel Task</button>
		</form>
		</td>   
		</tr>
		";
	...
	$taskid = $_POST['taskid'];
	mysql_query("UPDATE tasks SET pending = 0, status = 3 WHERE taskid='$taskid'")or die(mysql_error());
		if(mysql_errno()){
...
function viewHost($host_id) {
	...
	if(!$rows) {
		echo "<br /><h2>No Host Selected</h2><br />";
	} else {
	...
	while ($row = mysql_fetch_array($data_p)) {
		...
		$group_query = mysql_query("SELECT name FROM groups"); // Run your query
		...
		<h4>Group Membership</h4>
         ...
         <span class='caret'></span>
			...
			while ($grouprow = mysql_fetch_array($group_query)) {
	 	...
	 	<hr>
 	 	<h4>Task Info</h4>";
 		 if(($curtime-$tasktime) > 300) {     // Display task was over 5 mins ago.. just for testing purposes
			echo "<span title='Client NOT ACTIVE within last 5 minutes'><img src='images/nonactive.png' width='16' /></span>";
			}	
			else {
			echo "<span title='Client ACTIVE within last 5 minutes'><img src='images/active.png'  width='16' /></span>";		
